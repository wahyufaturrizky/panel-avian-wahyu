...
class Penghargaan extends CI_Controller {
	...
	private $tbl = 'dtb_new_our_achievement';
	...
	public function index(){
		...
		$data = [
			...
			'content' => $this->load->view('back/penghargaan/index' ,'',true),
		];
	...
	function get_data_penghargaan(){
		...
		$where_like['data'][] = array(
			'column' => 'id,picture,header,description,year',
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				if ($val->id>0) {
            		...
            		$img = "<img src='".base_url()."assets/picture/pic_penghargaan/".$val->picture."' style='height:52px;width:52px;'>";
	...
	function save_penghargaan($id=null){
		if (!$this->input->is_ajax_request()) {
			exit('No direct script access allowed');
		}

		$config['upload_path']          = './assets/picture/pic_penghargaan/';
		...
		if ($id != null && $id > 0) {
			...
			$old_path = 'assets/picture/pic_penghargaan/'.$old_img;
