...
class Berita extends CI_Controller {
	...
	public function index(){
		...
		$data = [
			...
			'content' => $this->load->view('back/berita/index' ,'',true),
		];
	...
	function get_data_berita(){
		...
		$where_like['data'][] = array(
			'column' => 'id,picture,header,description,location,date_create',
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				if ($val->id>0) {
            		...
            		$img = "<img src='".base_url()."assets/picture/pic_berita/".$val->picture."' style='height:52px;width:52px;'>";
	...
	function save_berita($id=null){
		if (!$this->input->is_ajax_request()) {
			exit('No direct script access allowed');
		}

		$config['upload_path']          = './assets/picture/pic_berita/';
		...
		if ($id != null && $id > 0) {
			...
			$old_path = 'assets/picture/pic_berita/'.$old_img;
