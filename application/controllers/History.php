...
class History extends CI_Controller {
	...
	private $tbl = 'dtb_new_timeline';
	private $tbl2= 'dtb_new_timeline_header';
	...
	public function index(){
		...
		$data = [
			...
			'content' => $this->load->view('back/sejarah/index' ,'',true),
		];
	...
	function get_data_sejarah(){
		...
		$where_like['data'][] = array(
			'column' => 'id,picture,year,description',
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				if ($val->id>0) {
            		...
            		$img = "<img src='".base_url()."assets/picture/sejarah/".$val->picture."' style='height:80px;width:80px;'>";
	...
	function save_history($id=null){
		if (!$this->input->is_ajax_request()) {
			exit('No direct script access allowed');
		}

		$config['upload_path']          = './assets/picture/sejarah/';
		...
		if ($id != null && $id > 0) {
			...
			$old_path = 'assets/picture/sejarah/'.$old_img;
