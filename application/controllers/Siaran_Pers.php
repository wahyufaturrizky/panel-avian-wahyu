...
class Siaran_Pers extends CI_Controller {
	...
	private $tbl = 'dtb_new_siaran_pers';
	...
	public function index(){
		...
		$data = [
			...
			'content' => $this->load->view('back/siaran_pers/index' ,'',true),
		];
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				if ($val->id>0) {
            		...
            		$img = "<img src='".base_url()."assets/picture/pic_siaran_pers/".$val->picture."' style='height:80px;width:80px;'>";
	...
	function save_siaran_pers($id=null){
		if (!$this->input->is_ajax_request()) {
			exit('No direct script access allowed');
		}

		$config['upload_path']          = './assets/picture/pic_siaran_pers/';
		...
		if ($id != null && $id > 0) {
			...
			$old_path = 'assets/picture/pic_siaran_pers/'.$old_img;
