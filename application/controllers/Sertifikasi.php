...
class Sertifikasi extends CI_Controller {
	...
	private $tbl = 'dtb_new_sertifikasi';
	...
	public function index(){
		...
		$data = [
			...
			'content' => $this->load->view('back/sertifikasi_kami/index' ,'',true),
		];
	...
	function get_data_sertifikasi_kami(){
		$select = '*';
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				if ($val->id>0) {
            		...
            		$img = "<img src='".base_url()."assets/picture/pic_sertifikasi_kami/".$val->picture."' style='height:52px;width:52px;'>";
	...
	function save_sertifikasi_kami($id=null){
		if (!$this->input->is_ajax_request()) {
			exit('No direct script access allowed');
		}

		$config['upload_path']          = './assets/picture/pic_sertifikasi_kami/';
		...
		if ($id != null && $id > 0) {
			...
			$old_path = 'assets/picture/pic_sertifikasi_kami/'.$old_img;
