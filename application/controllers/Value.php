...
class Value extends CI_Controller {
	...
	public function index(){
		...
		$data = [
			...
			'content' => $this->load->view('back/nilai_kami/index' ,'',true),
		];
	...
	function get_data_nilai_kami(){
		...
		$where_like['data'][] = array(
			'column' => 'id,picture,header,ordering,description',
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				if ($val->id>0) {
            		...
            		$img = "<img src='".base_url()."assets/picture/pic_nilai_kami/".$val->picture."' style='height:52px;width:52px;'>";
	...
	function save_nilai_kami($id=null){
