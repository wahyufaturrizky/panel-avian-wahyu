...
class Daftar_Aktivitas_CSR extends CI_Controller {
	
	private $tbl = 'dtb_new_csr';
	...
	public function index(){
		...
		$breadcrumb = [
			'0'=>'Dashboard' ,
			'1'=>'Frontend',
			'2'=>'Daftar Aktivitas CSR'
		];

		$data = [
			'title' => 'Daftar Aktivitas CSR' ,
			'breadcrumb' => $breadcrumb , 'b_url' => $url,
			'content' => $this->load->view('back/daftar_aktivitas_csr/index' ,'',true),
		];
	...
	function get_data_daftar_aktivitas_csr(){
		$select = '*';
		...
		$where_like['data'][] = array(
			'column' => 'id,picture,header,ordering,description,location',
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				if ($val->id>0) {
            		...
            		$img = "<img src='".base_url()."assets/picture/pic_daftar_aktivitas_csr/".$val->picture."' style='height:52px;width:52px;'>";
	...
	function save_daftar_aktivitas_csr($id=null){
		if (!$this->input->is_ajax_request()) {
			exit('No direct script access allowed');
		}

		$config['upload_path']          = './assets/picture/pic_daftar_aktivitas_csr/';
		...
		if ($id != null && $id > 0) {
			...
			$old_path = 'assets/picture/pic_daftar_aktivitas_csr/'.$old_img;
