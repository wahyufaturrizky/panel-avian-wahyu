...
class AIC extends CI_Controller {
	
	private $tbl = 'dtb_new_aic';
	...
	public function index(){
		...
		$breadcrumb = [
			'0'=>'Dashboard' ,
			'1'=>'Frontend',
			'2'=>'AIC (Avian Innovation)'
		];

		$data = [
			'title' => 'AIC (Avian Innovation)' ,
			'breadcrumb' => $breadcrumb , 'b_url' => $url,
			'content' => $this->load->view('back/aic/index' ,'',true),
		];
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				if ($val->id>0) {
            		...
            		$img = "<img src='".base_url()."assets/picture/pic_aic/".$val->picture."' style='height:80px;width:80px;'>";
	...
	function save_aic($id=null){
		if (!$this->input->is_ajax_request()) {
			exit('No direct script access allowed');
		}

		$config['upload_path']          = './assets/picture/pic_aic/';
		...
		if ($id != null && $id > 0) {
			...
			$old_path = 'assets/picture/pic_aic/'.$old_img;
